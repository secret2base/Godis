...
func (raft *Raft) Join(seed string) protocol.ErrorReply {
	...
	defer func(clientFactory clientFactory, peerAddr string, peerClient peerClient) {
		err := clientFactory.ReturnPeerClient(peerAddr, peerClient)
		if err != nil {
			logger.Error(err)
		}
	}(cluster.clientFactory, seed, seedCli)
	...
	}(cluster.clientFactory, leaderAddr, leaderCli)
